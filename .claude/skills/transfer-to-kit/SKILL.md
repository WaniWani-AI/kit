...
| `server/src/index.ts`, `api/index.ts`, `web/vite.config.ts`, `Dockerfile`, `vercel.json`, `tsconfig.json` | deleted, the kit owns all of it |
...
Deploying is `vercel deploy --prebuilt` from inside `.waniwani/`, which the
framework has already filled with a self-contained function bundle. The install
step never runs, so the deploy does not depend on the kit being published. Point
it at an existing project rather than creating one:

```bash
cd .waniwani
VERCEL_ORG_ID=team_... VERCEL_PROJECT_ID=prj_... vercel deploy --prebuilt --archive=tgz
```

Two things to expect there. A preview URL under Vercel's SSO protection answers
`Protected deployment` to any probe, so verifying a preview needs a protection
bypass token or a custom domain. And the emitted function pins `nodejs22.x`
whatever the project's Node setting says, because prebuilt output outranks it.
