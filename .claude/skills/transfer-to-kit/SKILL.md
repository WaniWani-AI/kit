...
| `vercel.json` | deleted, then regenerated by the first build (see Deploying) |
...
Deploying is a git push. The first `waniwani build` writes a `vercel.json` at the
app root that makes a git-connected project build the kit's output tree, and the
three lines in it are all load-bearing: read the README's deploy section before
editing it. An app that arrived with a `vercel.json` of its own keeps it, so
**a transfer has to replace the old one by hand**. The old shape is recognisable:
...
That points at an entry file the transfer deleted. Replace the whole file with
the generated one (delete it, run `waniwani build`, and the kit writes it).
...
Then read the emitted route table, which is where the one Vercel-specific trap
shows up:
...
serverless function of its own, and those sit in the filesystem layer; the route
ahead of it is what sends `/api/*` to the server the kit built instead. Without
it every endpoint answers a runtime error in production while every local check
passes.
