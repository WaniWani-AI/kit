 ...
 * discover:
 *
 *   node scripts/template-contract.mjs                    # the pinned template
 *   node scripts/template-contract.mjs --template ../t    # a local checkout
 *   node scripts/template-contract.mjs --template github:WaniWani-AI/mcp-distribution-template#beta
 *
...
// in exactly one place in this repo (see the note in packages/kit/cli/log.mjs).
import { bold, dim, green, red } from "../packages/kit/cli/log.mjs";

const REPO_ROOT = join(dirname(fileURLToPath(import.meta.url)), "..");
const KIT_DIR = join(REPO_ROOT, "packages/kit");
const CLI = join(KIT_DIR, "cli/index.mjs");
...
const app = resolve(REPO_ROOT, flags.app ?? "examples/oney");
 ...
 */
const template = flags.template?.startsWith("github:")
	? flags.template
	: flags.template
		? resolve(flags.template)
		: undefined;
 ...
 */
function run(command, args, { cwd = REPO_ROOT, reason, tolerate = false } = {}) {
...
if (nodeMajor < MINIMUM_NODE) {
	fail(
		`this needs Node ${MINIMUM_NODE} or newer, and is running ${process.versions.node}`,
		"the generated project declares the same floor in its engines field, and the framework's CLI does not load below it",
	);
}

console.log(`node      ${process.versions.node}`);
console.log(`template  ${template ?? "the commit packages/kit/cli/template.mjs pins"}`);
console.log(`app       ${app}`);
...
// `npm`, though this repo installs with bun: the script it runs is a plain
// `tsc -p tsconfig.json`, so the two are equivalent here, and npm ships with
// node. One less thing that has to be on PATH for the contract to be runnable.
...
run("node", [join(REPO_ROOT, "scripts/probe.mjs"), url], {
